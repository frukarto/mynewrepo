							...
							<xsl:for-each select="tns1:TransportFeasibilityParameters">
								...
								<xsl:for-each select="tns1:AvailabilityFlag">
									...
									<xsl:for-each select="$SLVT_finalmerge/sslvt:MergedSLVTMsg">
										...
										<xsl:for-each select="sslvt:AvailabilityFlag">
											<xsl:variable name="AvailabilityFlag1" select="."/>
											<xsl:variable name="ComparisonResult" select="$AvailabilityFlag = $AvailabilityFlag1"/>
											<xsl:for-each select="$TransportFeasibilityParameters/tns1:SatellitedID">
												...
												<xsl:for-each select="$MergedSLVTMsg/sslvt:SatellitedID">
													...
													<xsl:variable name="ComparisonResult1" select="$SatellitedID = $SatellitedID1"/>
													<xsl:variable name="AndResult" select="$ComparisonResult and $ComparisonResult1"/>
													<xsl:if test="string($AndResult)='true'">
												<TransportFeasibilityParameter>
													...
													<xsl:for-each select="$TransportFeasibilityParameters/tns1:BusinessAvailabilityFlag">
													...
													<xsl:for-each select="$TransportFeasibilityParameters/tns1:Polarization">
														<Polarization>
															<xsl:value-of select="."/>
														</Polarization>
													</xsl:for-each>
													<xsl:for-each select="$TransportFeasibilityParameters/tns1:Band">
														<Band>
															<xsl:value-of select="."/>
														</Band>
													</xsl:for-each>
													<xsl:for-each select="$TransportFeasibilityParameters/tns1:MinODUPower">
														<MinODUPower>
															<xsl:value-of select="."/>
														</MinODUPower>
													</xsl:for-each>
													<xsl:for-each select="$TransportFeasibilityParameters/tns1:MinAntennaSize">
														<MinAntennaSize>
															<xsl:value-of select="."/>
														</MinAntennaSize>
													</xsl:for-each>
													<xsl:for-each select="$TransportFeasibilityParameters/tns1:TransportName">
														<TransportName>
															<xsl:value-of select="."/>
														</TransportName>
													</xsl:for-each>
													...
													<!--<xsl:for-each select="$MergedSLVTMsg/sslvt:SLVTResponseMsg">
														<xsl:for-each select="sslvt:SLVTParametersList">
															<xsl:variable name="SLVTParametersList" select="."/>
															<xsl:for-each select="$TransportFeasibilityParameters/tns1:SatellitedID">
																<xsl:variable name="SatellitedID" select="."/>
																<xsl:for-each select="$MergedSLVTMsg/sslvt:SatellitedID">
																	...
																	<xsl:for-each select="$MergedSLVTMsg/sslvt:BeamID">
																		...
																		<xsl:for-each select="$TransportFeasibilityParameters/tns1:BeamID">
																			...
																			<xsl:variable name="ComparisonResult2" select="$BeamID = $BeamID1"/>
																			<xsl:for-each select="$TransportFeasibilityParameters/tns1:AvailabilityFlag">
																				<xsl:variable name="AvailabilityFlag2" select="."/>
																				<xsl:for-each select="$MergedSLVTMsg/sslvt:AvailabilityFlag">
																					<xsl:variable name="AvailabilityFlag3" select="."/>
																					<xsl:variable name="ComparisonResult3" select="$AvailabilityFlag2 = $AvailabilityFlag3"/>
																					<xsl:variable name="AndResult" select="$ComparisonResult1 and $ComparisonResult2"/>
																					<xsl:variable name="AndResult1" select="$AndResult and $ComparisonResult3"/>
																					<xsl:if test="string($AndResult1)='true'">
																						<SLVTParametersList>
																							<xsl:for-each select="$SLVTParametersList/slvt:SLVTParameters">
																								<SLVTParameters>
																									<xsl:for-each select="slvt:AntennaSize">
																										<AntennaSize>
																											<xsl:value-of select="."/>
