...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:core="http://www.hns.com/iag/schemas/core" xmlns:n1="http://www.hns.com/iag/schemas/partner" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:tns="http://soap.sforce.com/schemas/class/PartnersOrder" exclude-result-prefixes="core n1 xs">
		...
		<xsl:template match="n1:CreateCustomerRequest">
			<tns:CreateCustomerRequest>
				<tns:req>
					...
					<xsl:for-each select="n1:DistributorFlag">
						<tns:DistributorFlag>
							<xsl:value-of select="."/>
						</tns:DistributorFlag>
					</xsl:for-each>
					<xsl:for-each select="n1:DistributorId">
						<tns:DistributorId>
							<xsl:value-of select="."/>
						</tns:DistributorId>
					</xsl:for-each>
					...
					<xsl:for-each select="n1:InvoiceInfo">
						<tns:InvoiceInfo>
							<xsl:for-each select="n1:Branch">
								<tns:Branch>
									<xsl:value-of select="."/>
								</tns:Branch>
							</xsl:for-each>
							<xsl:for-each select="n1:BusinessBankAccountNumber">
								<tns:BusinessBankAccountNumber>
									<xsl:value-of select="."/>
								</tns:BusinessBankAccountNumber>
							</xsl:for-each>
							<xsl:for-each select="n1:BusinessBankReference">
								<tns:BusinessBankReference>
									<xsl:value-of select="."/>
								</tns:BusinessBankReference>
							</xsl:for-each>
							<xsl:for-each select="n1:BusinessType">
								<tns:BusinessType>
									<xsl:value-of select="."/>
								</tns:BusinessType>
							</xsl:for-each>
							...
							<xsl:for-each select="n1:TradeReference">
								<tns:TradeReference>
									<xsl:for-each select="n1:Fax">
										<tns:Fax>
											<xsl:value-of select="."/>
							...
							<xsl:for-each select="n1:YearsInBusiness">
								<tns:YearsInBusiness>
									<xsl:value-of select="."/>
								</tns:YearsInBusiness>
							</xsl:for-each>
